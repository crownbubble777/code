...
public class LocalAuthController {
	...
	private Map<String, Object> bindLocalAuth(HttpServletRequest request) {
		...
		// 从session中获取当前的用户信息(用户一旦通过微信登录后，便能获取到用户的信息)
		PersonInfo user = (PersonInfo) request.getSession().getAttribute("user");
		// 非空判断，要求账密码以及当前的用户session为空
	...
	private Map<String, Object> changeLocalPwd(HttpServletRequest request) {
		...
		if (userName != null && password != null && newPassword != null && user != null && user.getUserId() != null
			...
			try {
				// 查看原先的账号，查看与输入的账号是否一致，不一致则认为是非法操作
				LocalAuth localAuth = localAuthService.getLocalAuthByUserId(user.getUserId());
				if (localAuth == null || !localAuth.getUsername().equals(userName)) {
					// 不一致则直接退出
					modelMap.put("success", false);
					modelMap.put("errMsg", "输入的账号费本次登录的账号");
					return modelMap;
				}
				// 修改平台账号的用户密码
				...
				if (le.getState() == LocalAuthStateEnum.SUCCESS.getState()) {
					...
					modelMap.put("success", false);
					modelMap.put("errMsg", le.toString());
