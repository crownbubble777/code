$(function(){
		...
		.on(
				...
				function(e){
					...
					if(target.hasClass('edit')){
						//如果class edit则点击即进入商品信息编辑页面，并带有productId参数
						window.location.href='/o2o/shopadmin/productoperation?productId='
							+e.currentTarget.dataset.id;
					}else if(target.hasClass('status')){
						//如果有class status则调用后台功能上/下架相关商品，并带有porductId参数
	...
	function changeItemStatus(id,enableStatus){
		...
		$.confirm('确认么?',function(){
			//上下架相关商品，如果确定的话，就直接去调用modifyproduct
			$.ajax({
				url:statusUrl,
				type:'POST',
				data:{
